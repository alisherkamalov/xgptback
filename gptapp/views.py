...
class AskView(APIView):
    def post(self, request):
        try:
            ...
            # Запрос к API g4f
            response = g4f.ChatCompletion.create(model='gpt-3.5-turbo',provider=g4f.Provider.Bing, messages=[{"role": "user", "content": question}])
            ...
            if not gpt_response:
                return Response({"error": "Failed to get a response from GPT-3.5"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
